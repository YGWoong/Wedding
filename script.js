...
(function () {
    ...
    function setMetaTags() {
        ...
        setMeta('property', 'og:image', 'images/mt/1.jpg');
    ...
    function initCurtain() {
        ...
        btn.addEventListener('click', () => {
            ...
            // 1. [핵심] 화면 진입 시 음소거로 자동 재생된 오디오를,
            // 사용자가 화면을 첫 클릭/터치하는 순간 음소거를 해제(소리가 나오게) 합니다.
            function triggerAudio() {
                if (bgm.muted) {
                    bgm.muted = false; // 음소거 해제
                    bgm.play().catch(function (error) {
                        // 혹시라도 완전히 멈춰있던 경우를 대비해 다시 play 실행
                        bgm.play();
                    });
                    toggleBtn.innerText = "⏸️ 음악 정지";
                }
                // 한 번 소리가 켜지면 이 이벤트 리스너들은 제거합니다 (중복 실행 방지)
                document.removeEventListener('click', triggerAudio);
                document.removeEventListener('touchstart', triggerAudio);
            }

            // 화면 아무 데나 클릭하거나 터치하면 triggerAudio 함수 실행
            document.addEventListener('click', triggerAudio);
            document.addEventListener('touchstart', triggerAudio);

            // 2. 버튼 클릭 시 재생/정지 토글 기능
            toggleBtn.addEventListener('click', function (event) {
                // 버튼 클릭이 상단의 document 클릭 이벤트로 번지는 것을 막아줍니다
    ...
    function initFallingLeaves() {
        ...
        const leafColors = [{
                fill: 'rgba(139, 158, 126, 0.6)',
                stroke: 'rgba(74, 94, 59, 0.3)'
            }, // sage green
            {
                fill: 'rgba(74, 94, 59, 0.5)',
                stroke: 'rgba(58, 75, 46, 0.3)'
            }, // forest green
            {
                fill: 'rgba(168, 184, 158, 0.5)',
                stroke: 'rgba(139, 158, 126, 0.3)'
            }, // light sage
            {
                fill: 'rgba(180, 165, 120, 0.5)',
                stroke: 'rgba(139, 115, 85, 0.3)'
            }, // golden
            {
                fill: 'rgba(160, 175, 130, 0.5)',
                stroke: 'rgba(100, 120, 70, 0.3)'
            }, // yellow-green
        ];
    ...
    function initGreeting() {
        ...
        function parentLine(father, mother, fatherDeceased, motherDeceased) {
            ...
            return `<span class="${fd}">${father}</span><span class="parent-dot">●</span><span class="${md}">${mother}</span>`;
        }
    ...
    function initGallery(galleryImages) {
        ...
        galleryImages.forEach((src, i) => {
            ...
            // modalDiv.className = 'photo-modal__div';
            // modalDiv.id = `modalImage_${i + 1}`;
            // modalDiv.innerHTML = `
            //     <picture class="photo-modal__img">
            //         <source srcset="${src.webp}" type="image/webp">
            //         <img src="${src.img}" alt="모달 사진 ${i + 1}" loading="lazy" draggable="false">
            //     </picture>`;
    ...
    function showModalImage() {
        ...
        $('#modalCounter').textContent = `${modalIndex + 1} / ${CONFIG.gallerySize}`;

        $('#modalPrev').style.display = modalIndex > 0 ? '' : 'none';
        $('#modalNext').style.display = modalIndex < CONFIG.gallerySize - 1 ? '' : 'none';

        // document.querySelectorAll('.photo-modal__div').forEach(element => {
        //     element.classList.remove('show-image');
        // });
        // $(`#modalImage_${modalIndex + 1}`).classList.add('show-image');
        ...
        setTimeout(() => {
            ...
            const placeholder = modalDiv.querySelector('.loading-placeholder_modal');
            if (placeholder)
                placeholder.remove();

            viewer.style.backgroundImage = `url('${modalImages[modalIndex].img}')`;
        }, 100);
    }
    ...
    function showLoadingModal() {
        ...
        const placeholderHTML = '<div class="loading-placeholder_modal"><span class="loading-dot"></span><span class="loading-dot"></span><span class="loading-dot"></span></div>';

        if (modalDiv)
            modalDiv.insertAdjacentHTML('beforeend', placeholderHTML);
    }

    function modalNavigate(dir) {
        const newIndex = modalIndex + dir;
        if (newIndex >= 0 && newIndex < CONFIG.gallerySize) {
            modalIndex = newIndex;
            showModalImage();
        }
    }
